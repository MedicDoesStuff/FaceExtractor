...
# Hardcode zig_bin path for Colab
zig_bin = Path('/content/zig/zig')
...
def compile_o(ispc_file: Path):
    ...
    args = [str(ispc_bin), str(ispc_file), '-o', str(ispc_o_filepath),
            '-h', str(ispc_h_filepath), '-I', str(include_root), '--pic']
